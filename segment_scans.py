...
def join_2_part_letters_OLD(components, CCA_info, numpy_file):  # this function joins the tittles in i and j to their stubs
    labels = CCA_info[1]
        ...
        if comp_a not in used_components:
            for comp_b in broken_components:
                if comp_b not in used_components:
                    if (
                            comp_a.x - 20 < comp_b.x < comp_a.x + 20) and comp_a.y - 30 < comp_b.y < comp_a.y + 30 and comp_a != comp_b:  # compares all tittles to the parameters they must meet to be joined
